    ...
    <nav class="sticky top-0 z-40 w-full border-b bg-background/80 backdrop-blur-xl transition-all duration-300">
        ...
        <div class="flex justify-between h-16 items-center">
            ...
            <div class="hidden md:flex items-center gap-1">
              ...
              <template v-if="isAdmin">
                <Link :href="route('dashboard')">
                  <Button :class="[route().current('dashboard') ? 'bg-muted text-foreground' : 'text-muted-foreground']"
              ...
              <template v-else>
                <Link :href="route('products.index')">
                  <Button :class="[route().current('products.*') ? 'bg-muted text-foreground' : 'text-muted-foreground']"
                          class="rounded-lg font-bold"
                          variant="ghost">
                ...
                <Link :href="route('orders.index')">
                  <Button :class="[route().current('orders.*') ? 'bg-muted text-foreground' : 'text-muted-foreground']"
                          class="rounded-lg font-bold"
                          variant="ghost">
          ...
          <div class="flex items-center gap-2">
            ...
            <Link v-if="!isAdmin" :href="route('cart.index')" class="relative">
              <Button class="rounded-full text-muted-foreground hover:text-foreground" size="icon" variant="ghost">
                <ShoppingCart class="size-5"/>
                <span v-if="cart.count > 0" :key="cart.count"
                      class="absolute -top-1 -right-1 size-5 bg-primary text-[10px] font-black text-primary-foreground rounded-full flex items-center justify-center border-2 border-background animate-in zoom-in-50 duration-300">
                                    {{ cart.count }}
                                </span>
              </Button>
            </Link>
            ...
            <div class="flex items-center">
              <DropdownMenu>
                <DropdownMenuTrigger as-child>
                  <Button class="p-1 rounded-full hover:bg-muted transition-colors" variant="ghost">
                    <Avatar class="size-8 border-2 border-border transition-transform hover:scale-105">
                      <AvatarImage src=""/>
                      <AvatarFallback class="bg-muted text-foreground text-[10px] font-black">{{
                          initials
                        }}
                      </AvatarFallback>
                    </Avatar>
                ...
                <DropdownMenuContent align="right" class="w-56 rounded-md shadow-premium mt-2 border-border p-1">
                  <div class="px-2 py-2 flex flex-col">
                    ...
                    <p class="text-[10px] font-bold text-muted-foreground truncate">{{
                        $page.props.auth.user.email
                      }}</p>
                  </div>
                  <DropdownMenuSeparator/>
                  <DropdownMenuLabel
                      class="px-2 py-1.5 text-xs font-black uppercase tracking-widest text-muted-foreground">Account
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator/>
                  <DropdownMenuItem
                      class="rounded-lg my-0.5 flex items-center gap-2 font-bold cursor-pointer text-popover-foreground"
                  ...
                  <DropdownMenuItem v-if="isAdmin" as-child
                                    class="rounded-lg my-0.5 flex items-center gap-2 font-bold cursor-pointer text-popover-foreground">
                    <a class="flex items-center gap-2" href="/horizon" target="_blank">
                  ...
                  <DropdownMenuSeparator/>
                  <DropdownMenuItem
                      class="rounded-lg my-0.5 flex items-center gap-2 font-bold text-destructive focus:text-destructive focus:bg-destructive/5 cursor-pointer"
            ...
            <!-- Mobile Menu Button -->
            <div class="md:hidden">
              <Button class="rounded-lg" size="icon" variant="ghost"
                      @click="showingNavigationDropdown = !showingNavigationDropdown">
                <Menu v-if="!showingNavigationDropdown"
                      class="size-6 transition-transform hover:rotate-90 duration-300"/>
      ...
      >
        <div v-if="showingNavigationDropdown"
             class="md:hidden border-b bg-card absolute w-full shadow-2xl rounded-b-3xl">
          <div class="px-4 py-6 space-y-2">
            <!-- Admin Mobile -->
            <template v-if="isAdmin">
              <Link :href="route('dashboard')">
                <Button :class="route().current('dashboard') ? 'bg-muted' : ''" class="w-full justify-start h-12 rounded-lg font-bold"
                        variant="ghost">
            ...
            <template v-else>
              <Link :href="route('products.index')">
                <Button :class="route().current('products.*') ? 'bg-muted' : ''" class="w-full justify-start h-12 rounded-lg font-bold"
                        variant="ghost">
                  <Package class="size-5 mr-3"/>
                  Marketplace
                </Button>
              </Link>
              <Link :href="route('orders.index')">
                <Button :class="route().current('orders.*') ? 'bg-muted' : ''" class="w-full justify-start h-12 rounded-lg font-bold"
                        variant="ghost">
              ...
              <Link :href="route('cart.index')">
                <Button :class="route().current('cart.*') ? 'bg-muted' : ''" class="w-full justify-start h-12 rounded-lg font-bold"
                        variant="ghost">
                  <ShoppingCart class="size-5 mr-3"/>
                  Cart ({{ cart.count }})
                </Button>
              </Link>
            </template>
    ...
    <Dialog :open="successModalOpen" @update:open="successModalOpen = $event">
      <DialogContent class="sm:max-w-md rounded-xl border-none shadow-2xl p-10 overflow-hidden">
        ...
        <div class="flex flex-col items-center text-center">
          ...
          <DialogHeader>
            <DialogTitle class="text-3xl font-black text-foreground tracking-tight leading-none mb-2">Order Confirmed!
            </DialogTitle>
          ...
          <div class="mt-8 w-full space-y-4 relative z-10">
            <div class="flex items-center justify-center gap-3">
              ...
              <span class="text-xs font-bold text-muted-foreground uppercase tracking-widest">
                                Redirecting in <span
                  class="bg-muted px-2 py-0.5 rounded text-primary font-black ml-1 tabular-nums">{{ countdown }}</span>
                            </span>
            </div>

            <Button class="w-full h-12 rounded-lg font-bold text-base shadow-lg shadow-primary/20"
                    @click="successModalOpen = false; router.visit(route('products.index'))">
              Keep Shopping
            </Button>
          </div>
        </div>
