...
<template>
  <Card
      class="group overflow-hidden border-none shadow-soft hover:shadow-hover transition-all duration-500 bg-card/40 backdrop-blur-sm">
    ...
    <CardContent class="p-5 flex flex-col gap-2.5">
      ...
      <div v-if="product.tags?.length" class="flex flex-wrap gap-1.5 mt-1">
                <span v-for="tag in product.tags.slice(0, 3)" :key="tag"
                      class="text-[10px] font-semibold text-muted-foreground bg-muted px-2 py-0.5 rounded-sm">
                    #{{ tag }}
                </span>
      </div>

      <div
          class="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60 mt-2">
    ...
    <CardFooter class="p-5 pt-0">
      ...
      >
        ...
        <ShoppingCart v-else
                      class="mr-2 size-4 transition-transform group-hover/btn:-translate-y-0.5 group-hover/btn:translate-x-0.5"/>
