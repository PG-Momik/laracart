    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <div class="size-14 bg-card rounded-2xl shadow-soft border border-muted-foreground/10 flex items-center justify-center">
        ...
        <div class="max-w-5xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            <div v-if="orders.data.length > 0" class="space-y-8">
                ...
                >
                    <CardHeader class="p-6 sm:p-8 pb-0">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                            <div class="flex items-center gap-5">
                                <div class="size-16 bg-muted/30 rounded-2xl flex items-center justify-center border border-muted-foreground/5 group-hover:bg-primary/5 group-hover:border-primary/10 transition-colors duration-500">
                            ...
                            <div class="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-4">
                                ...
                                >
                                    <span :class="['size-1.5 rounded-full mr-2 animate-pulse', order.status.color === 'green' ? 'bg-green-500' : (order.status.color === 'blue' ? 'bg-primary' : 'bg-muted-foreground')]"></span>
                                    {{ order.status.label }}
                                </Badge>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8">
                            ...
                            <div v-if="order.items_preview" class="flex items-center gap-3">
                                <div class="flex -space-x-4">
                                    <div v-for="(item, idx) in order.items_preview" :key="idx" class="relative group/avatar">
                                        <Avatar class="size-14 border-4 border-card rounded-2xl group-hover:translate-y-[-4px] transition-transform duration-300 shadow-soft">
                                ...
                                <div v-if="order.item_count > order.items_preview.length" class="pl-2">
                                    <Badge variant="secondary" class="h-14 px-4 rounded-2xl font-black text-muted-foreground border-dashed border-2">
                            ...
                            <div class="flex items-center gap-3 w-full sm:w-auto">
                                <Link :href="route('orders.show', order.id)" class="w-full sm:w-auto">
                                    <Button size="lg" variant="outline" class="w-full h-14 rounded-2xl font-black text-xs uppercase tracking-widest gap-2 bg-background/50 hover:bg-primary hover:text-white transition-all duration-300 border-muted-foreground/10">
                                        Review Details
                    ...
                    <div class="bg-muted/30 px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-muted-foreground/5">
                        <div class="flex items-center gap-2">
                            <CheckCircle2 class="size-4 text-green-500" />
                            <span class="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Verified Transaction</span>
                        </div>
            ...
            <div v-else class="text-center py-24 bg-card/40 rounded-[3rem] border-2 border-dashed border-muted-foreground/20 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                <div class="size-24 bg-muted/20 rounded-3xl flex items-center justify-center mb-8 mx-auto">
                ...
                <Link :href="route('products.index')">
                    <Button size="lg" class="h-16 px-12 rounded-2xl font-black text-lg bg-primary shadow-2xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all group">
                        Enter Marketplace
