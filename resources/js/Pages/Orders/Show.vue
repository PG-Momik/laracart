    ...
    <div class="max-w-5xl mx-auto py-12 px-4 sm:px-6 lg:px-8 space-y-8">
      ...
      <Card class="border-none shadow-premium bg-card/60 backdrop-blur-xl overflow-hidden rounded-2xl">
        <CardContent class="p-8 sm:p-12">
          <!-- Meta Info Header -->
          <div
              class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-8 pb-10 border-b border-muted-foreground/10">
            <div class="space-y-3">
            ...
            <div class="flex flex-col sm:items-end gap-3 w-full sm:w-auto">
              ...
              >
                <span
                    :class="['size-1.5 rounded-full mr-2 animate-pulse', orderData.status.color === 'green' ? 'bg-green-500' : (orderData.status.color === 'blue' ? 'bg-primary' : 'bg-muted-foreground')]"></span>
          ...
          <div class="pt-12 space-y-8">
            ...
            <div class="space-y-8">
              <div v-for="item in orderData.items" :key="item.id"
                   class="flex gap-8 items-center bg-muted/10 p-4 rounded-xl border border-muted-foreground/5 hover:bg-muted/20 transition-all group">
                <div class="relative flex-shrink-0">
                  <div
                      class="size-24 sm:size-28 rounded-xl border border-muted-foreground/10 bg-white flex items-center justify-center p-4 shadow-soft overflow-hidden group-hover:scale-105 transition-transform duration-500">
                    <img
                  ...
                  <Badge
                      class="absolute -top-3 -right-3 size-8 rounded-lg bg-primary shadow-lg border-4 border-card font-black text-xs flex items-center justify-center">
                    {{ item.quantity }}
                  </Badge>
                </div>
                <div class="flex-grow space-y-2">
                  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                    <h4 class="text-xl font-black text-foreground tracking-tight group-hover:text-primary transition-colors">
                      {{ item.product.name }}</h4>
                    <p class="text-xl font-black text-foreground">{{ item.formatted_subtotal }}</p>
                  </div>
                  <div class="flex items-center gap-4">
                    <Badge class="text-[9px] font-black uppercase tracking-widest px-2" variant="secondary">
          ...
          <div class="mt-16 pt-12 border-t border-muted-foreground/10 flex justify-end">
            <Card class="w-full sm:max-w-md border-none bg-muted/30 p-8 rounded-xl">
              <div class="space-y-4">
                <div
                    class="flex justify-between items-center text-xs font-black text-muted-foreground uppercase tracking-widest">
                  <span>Value</span>
        ...
        <div
            class="bg-primary p-8 sm:p-12 text-center sm:text-left flex flex-col sm:flex-row items-center justify-between gap-8 relative overflow-hidden">
          <div class="relative z-10 space-y-2">
            ...
            <p class="text-primary-foreground/70 font-medium text-sm">Our logistics experts are preparing your shipment
              for express delivery.</p>
          </div>
          <Link :href="route('products.index')" class="relative z-10 w-full sm:w-auto">
            <Button class="w-full sm:px-10 h-14 rounded-md font-black text-sm uppercase tracking-widest shadow-xl shadow-black/10 bg-secondary" size="lg"
                    variant="primary">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div
            class="bg-card/40 p-6 rounded-xl border border-muted-foreground/10 flex items-start gap-4 shadow-soft hover:shadow-premium transition-all duration-500">
          ...
          <div>
            <h4 class="text-xs font-black text-foreground uppercase tracking-widest mb-1.5 md:mb-1">Priority
              Support</h4>
            <p class="text-[10px] text-muted-foreground font-black leading-relaxed uppercase tracking-tight">Access 24/7
              dedicated assistance for your purchase queries.</p>
          </div>
        </div>
        <div
            class="bg-card/40 p-6 rounded-xl border border-muted-foreground/10 flex items-start gap-4 shadow-soft hover:shadow-premium transition-all duration-500">
          ...
          <div>
            <h4 class="text-xs font-black text-foreground uppercase tracking-widest mb-1.5 md:mb-1">Secure
              Integrity</h4>
            <p class="text-[10px] text-muted-foreground font-black leading-relaxed uppercase tracking-tight">End-to-end
              encryption ensures your financial metadata stays private.</p>
          </div>
        </div>
        <div
            class="bg-card/40 p-6 rounded-xl border border-muted-foreground/10 flex items-start gap-4 shadow-soft hover:shadow-premium transition-all duration-500">
          ...
          <div>
            ...
            <p class="text-[10px] text-muted-foreground font-black leading-relaxed uppercase tracking-tight">A
              verifiable digital copy has been dispatched to your mailbox.</p>
          </div>
        </div>
      </div>
    </div>
