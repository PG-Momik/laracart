        ...
        <template #header>
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em]">Administrative Command & Intelligence</p>
                </div>
            </div>
        </template>
                ...
                <div class="lg:col-span-4 space-y-6">
                    <Card class="border-none shadow-premium bg-card/50 backdrop-blur-xl rounded-3xl overflow-hidden border-t border-white/5">
                        <CardHeader class="p-8">
                            ...
                            <CardDescription class="text-[10px] uppercase font-bold text-muted-foreground mt-1">Execute specialized system operations</CardDescription>
                        </CardHeader>
                        <CardContent class="p-8 pt-0 space-y-6">
                            <div class="space-y-4">
                                <div class="space-y-2">
                                    ...
                                    <Select v-model="selectedCommand">
                                        ...
                                        <SelectContent class="rounded-md border-border">
                                            <SelectItem v-for="cmd in availableCommands" :key="cmd.value" :value="cmd.value" class="rounded-lg font-bold">
                                ...
                                <div v-if="['app:send-restock-mail', 'app:send-stock-alert', 'app:send-daily-report'].includes(selectedCommand)" class="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                                    ...
                                    />
                                    <p v-if="commandForm.errors.email" class="text-[10px] font-bold text-destructive uppercase animate-in fade-in slide-in-from-top-1">{{ commandForm.errors.email }}</p>
                                </div>

                                <!-- Dynamic Parameters -->
                                <div v-if="selectedCommand === 'products:fetch'" class="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                    <div class="space-y-2">
                                        <Label class="text-[10px] font-black uppercase tracking-wider opacity-50">Fetch Limit</Label>
                                        <Input type="number" v-model="commandForm.params['--limit']" class="h-12 rounded-md bg-muted/30 border-none font-bold no-spinner" />
                                        <p v-if="commandForm.errors['params.--limit']" class="text-[10px] font-bold text-destructive uppercase">{{ commandForm.errors['params.--limit'] }}</p>
                                    </div>
                                </div>

                                <div v-if="selectedCommand === 'app:send-restock-mail' || selectedCommand === 'app:send-stock-alert'" class="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                    <div class="space-y-2">
                                        ...
                                        <Select v-model="commandForm.params.product_id">
                                            <SelectTrigger :class="['h-12 rounded-md bg-muted/30 border-none font-bold', commandForm.errors['params.product_id'] && 'ring-2 ring-destructive']">
                                                <SelectValue placeholder="Select a product" />
                                            </SelectTrigger>
                                            <SelectContent class="rounded-md border-border">
                                                <SelectItem v-for="product in products" :key="product.id" :value="product.id.toString()" class="rounded-lg font-bold">
                                                    {{ product.name }}
                                                </SelectItem>
                                            </SelectContent>
                                        </Select>
                                        <p v-if="commandForm.errors['params.product_id']" class="text-[10px] font-bold text-destructive uppercase">{{ commandForm.errors['params.product_id'] }}</p>
                                    </div>
                                    ...
                                    <div v-if="selectedCommand === 'app:send-restock-mail'" class="space-y-2">
                                        ...
                                        <Input type="number" v-model="commandForm.params.quantity" class="h-12 rounded-md bg-muted/30 border-none font-bold no-spinner" />
                                        <p v-if="commandForm.errors['params.quantity']" class="text-[10px] font-bold text-destructive uppercase">{{ commandForm.errors['params.quantity'] }}</p>
                                    </div>
                                    ...
                                    <div v-if="selectedCommand === 'app:send-stock-alert'" class="space-y-2">
                                        ...
                                        <Input v-model="commandForm.params['--status']" class="h-12 rounded-md bg-muted/30 border-none font-bold" />
                                        <p v-if="commandForm.errors['params.--status']" class="text-[10px] font-bold text-destructive uppercase">{{ commandForm.errors['params.--status'] }}</p>
                                    </div>
                                </div>
