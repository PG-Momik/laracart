...
<template>
  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center gap-4">
        <Link :href="route('products.index')">
          <Button class="rounded-full hover:bg-muted" size="icon" variant="ghost">
    ...
    <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div class="lg:col-span-8 space-y-6">
            <Card class="border-none shadow-soft bg-card/50 backdrop-blur-sm overflow-hidden">
              <CardHeader class="pb-2">
                <CardTitle class="text-lg font-black uppercase tracking-widest text-muted-foreground">Order Items
                </CardTitle>
              </CardHeader>
              <CardContent class="p-0">
                <ul class="divide-y divide-muted-foreground/5" role="list">
                  <li v-for="item in cart.items" :key="item.id"
                      class="p-6 transition-all duration-300 hover:bg-muted/30 group">
                    <div class="flex gap-6">
                      <!-- Product Image -->
                      <div
                          class="relative size-24 sm:size-32 bg-muted/20 rounded-md overflow-hidden flex-shrink-0 flex items-center justify-center p-4">
                        <img
                      ...
                      <div class="flex-1 flex flex-col justify-between py-1">
                        <div class="space-y-1">
                          <div class="flex justify-between items-start">
                            <h3 class="text-lg font-black text-foreground group/link">
                              <Link :href="route('products.show', item.product.id)"
                                    class="group-hover/link:text-primary transition-colors">
                                {{ item.product.name }}
                              </Link>
                            </h3>
                          ...
                          <p class="text-sm text-muted-foreground line-clamp-1 italic max-w-sm mt-1">
                        ...
                        <div class="flex items-center justify-between mt-4">
                          ...
                          <div class="flex items-center gap-2">
                            <div
                                :class="['size-1.5 rounded-full ring-4 ring-opacity-20', item.product.stock_quantity > 0 ? 'bg-green-500 ring-green-500' : 'bg-destructive ring-destructive']"></div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div class="flex items-center gap-3 p-4 bg-primary/5 rounded-md border border-primary/10">
          ...
          <div class="lg:col-span-4 sticky top-24">
            <Card class="border-none shadow-premium bg-card/50 backdrop-blur-xl overflow-hidden">
              ...
              <CardContent class="space-y-6">
                ...
                <div class="flex justify-between items-center">
                  ...
                  <span class="text-3xl font-black text-primary transition-all duration-500">${{
              ...
              <CardFooter class="flex flex-col gap-3 p-6 pt-0">
                ...
                >
                  <Button
                      class="w-full h-14 rounded-md font-black text-lg transition-all hover:scale-[1.02] active:scale-95 group"
                      size="lg"
                  >
                ...
                <Link :href="route('products.index')" class="w-full">
                  <Button class="w-full rounded-lg font-bold text-muted-foreground hover:text-primary transition-colors uppercase tracking-widest text-[10px]"
                          variant="ghost">
                    Continue Shopping
                  </Button>
                </Link>
            ...
            <div class="mt-6 text-center">
              <p class="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Have a promo code? Add
                it at checkout.</p>
            </div>
          </div>
        </div>
      ...
      <!-- Empty Cart State -->
      <div v-else
           class="min-h-[60vh] flex flex-col items-center justify-center text-center px-4 animate-in fade-in slide-in-from-bottom-4 duration-1000">
        ...
        <p class="text-muted-foreground max-w-sm mb-12 font-medium">It feels a bit lonely here. Let's find some products
          to fill it up!</p>

        <Link :href="route('products.index')">
          <Button class="h-14 px-12 rounded-md font-black text-lg shadow-2xl shadow-primary/20 group" size="lg">
            Explore Marketplace
