...
<template>
  ...
  <div
      class="h-12 flex items-center justify-center relative overflow-hidden sticky top-0 z-50 shadow-lg bg-destructive/10 border-b border-destructive/20 backdrop-blur-md">
    <div class="relative flex items-center gap-3 px-4">
      <span
          class="bg-destructive text-destructive-foreground px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-lg">NOTICE</span>
      <strong class="text-xs font-bold text-foreground opacity-90">Payment processors are restricted in this region
        (Nepal). Simulated mode active.</strong>
    </div>
  </div>
  ...
  <div class="min-h-screen bg-background flex flex-col lg:grid lg:grid-cols-12 relative overflow-hidden">
    ...
    >
      <div v-if="processing"
           class="fixed inset-0 z-[100] bg-background/95 backdrop-blur-2xl flex flex-col items-center justify-center p-8 text-center">
        ...
        <div class="max-w-xs space-y-4">
          ...
          <div
              class="flex items-center justify-center gap-2 px-4 py-2 bg-muted/50 rounded-full border border-muted-foreground/10">
            <Lock class="size-3.5 text-primary"/>
            <span
                class="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Encrypted Channel</span>
          </div>
        </div>
      </div>
    </transition>
    ...
    <div
        class="lg:col-span-5 bg-muted/30 p-8 lg:p-16 lg:min-h-screen border-r border-muted-foreground/5 relative overflow-y-auto">
      <div class="max-w-lg mx-auto lg:mr-0 lg:ml-auto w-full space-y-12">
        <Link :href="route('cart.index')">
          <Button class="text-muted-foreground hover:text-primary transition-colors gap-2 -ml-4 px-4 font-bold uppercase tracking-widest text-[10px]"
                  variant="ghost">
        ...
        <div class="space-y-6">
          <div v-for="item in cart.items" :key="item.id" class="flex items-center gap-4 group">
            <div class="relative flex-shrink-0">
              <div
                  class="size-20 bg-card rounded-md border border-muted-foreground/10 flex items-center justify-center p-3 shadow-soft group-hover:scale-105 transition-transform">
                <img :alt="item.product?.name" :src="item.product?.image_url" class="size-full object-contain"/>
              </div>
              <span
                  class="absolute -top-2 -right-2 size-6 bg-primary text-primary-foreground text-[10px] font-black rounded-lg flex items-center justify-center border-2 border-background shadow-lg">
                                {{ item.quantity }}
                            </span>
            </div>
        ...
        <div class="space-y-4 pt-8 border-t border-muted-foreground/10">
          ...
          <div class="flex justify-between items-center text-sm font-bold">
            ...
            <Badge
                class="bg-green-500/10 text-green-600 hover:bg-green-500/20 border-green-500/20 font-black text-[10px]">
              FREE
            </Badge>
          </div>
    ...
    <div class="lg:col-span-7 p-8 lg:p-20 flex flex-col bg-background relative overflow-y-auto">
      <div class="w-full max-w-md mx-auto space-y-12">
        ...
        <div class="flex gap-4 p-2 bg-muted/40 rounded-xl border border-muted-foreground/10">
          ...
          >
            <CreditCard
                :class="['size-5 transition-colors', paymentMethod === 'stripe' ? 'text-primary' : 'text-muted-foreground']"/>
            <span>Card Payment</span>
            <div v-if="paymentMethod === 'stripe'"
                 class="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-1 bg-primary rounded-full"></div>
          </button>
          <button
              :class="[
                            'flex-1 flex gap-3 items-center justify-center py-4 rounded-md text-sm font-black transition-all relative overflow-hidden group',
          ...
          >
            <Wallet
                :class="['size-5 transition-colors', paymentMethod === 'paypal' ? 'text-[#0070e0]' : 'text-muted-foreground']"/>
          ...
          <div v-if="paymentMethod === 'stripe'" key="stripe" class="space-y-8">
            <form class="space-y-6" @submit.prevent="handleSubmit">
              <div class="space-y-2.5">
                <Label class="text-xs font-black uppercase tracking-widest text-muted-foreground ml-1">Contact
                  Information</Label>
                <Input v-model="email" class="h-14 rounded-md border-muted-foreground/10 bg-muted/20 focus-visible:ring-primary px-5 font-bold" placeholder="email@example.com"
                       type="email"/>
              </div>

              <div class="space-y-2.5">
                <div class="flex justify-between items-center ml-1">
                  <Label class="text-xs font-black uppercase tracking-widest text-muted-foreground">Payment
                    Details</Label>
                ...
                <div
                    class="space-y-0 divide-y divide-muted-foreground/10 border border-muted-foreground/10 rounded-md overflow-hidden shadow-soft bg-muted/5">
                  <div class="h-14 flex items-center px-5 gap-3 bg-background/50">
                    <CreditCard class="size-5 text-muted-foreground"/>
                    <input class="flex-grow bg-transparent border-none p-0 text-base font-bold tracking-widest pointer-events-none" disabled type="text"
                           value="4242 4242 4242 4242"/>
                  </div>
                  <div class="flex h-14 bg-background/30">
                    <input class="w-1/2 px-5 bg-transparent border-r border-muted-foreground/10 text-base font-bold pointer-events-none text-center rounded-bl-2xl" disabled type="text"
                           value="12 / 28"/>
                    <input class="w-1/2 px-5 bg-transparent text-base font-bold pointer-events-none text-center rounded-br-2xl" disabled type="text"
                           value="123"/>
                  </div>
                </div>
              ...
              <div class="space-y-2.5">
                <Label class="text-xs font-black uppercase tracking-widest text-muted-foreground ml-1">Cardholder
                  Name</Label>
                <Input v-model="cardName" class="h-14 rounded-md border-muted-foreground/10 bg-muted/20 focus-visible:ring-primary px-5 font-bold uppercase" placeholder="CARDHOLDER NAME"
                       type="text"/>
              </div>

              <Button class="w-full h-16 rounded-md font-black text-lg shadow-2xl shadow-primary/30 transition-all hover:scale-[1.02] active:scale-95 group"
                      type="submit">
          ...
          <div v-else key="paypal" class="space-y-8 animate-in fade-in slide-in-from-right-8 duration-500">
            <div class="bg-primary/5 border border-primary/20 rounded-xl p-6 flex items-start gap-4">
              ...
              <p class="font-bold text-muted-foreground leading-relaxed text-sm">
                You will be redirected to PayPal's secure portal to authorize this transaction. Lara Cart never stores
        ...
        <div class="pt-8 text-center space-y-4">
          <div
              class="inline-flex items-center gap-1.5 px-3 py-1 bg-green-500/5 text-green-600 rounded-full border border-green-500/10">
          ...
          <p class="text-[10px] text-muted-foreground font-medium leading-relaxed max-w-sm mx-auto">
            Your transaction is processed using secure encryption. Lara Cart utilizes advanced security checks for every
            purchase.
          </p>
        </div>
      </div>
